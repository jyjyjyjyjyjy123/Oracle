...
OraclesServiceEX > 데이터베이스
OracleXETNSListener > 리스너(클라이언트 도구와 연결)

SQL Developer
    데이터베이스 클라이언트 툴
    눈에 안보이는 오라클에 연결해서 조작하는 프로그램
    개발자 <-> 중계인(데이터베이스 클라이언트) <-> 오라클(데이터베이스 서버)
    새로 생성시 이름 > (오라클깔린 컴퓨터 이름).(접속하는 사용자)
...
ANSI-SQL
    ...
    2. DML - Data <anipulation Language
        데이터 조작어
        데이터를 추가/수정/삭제/조회하는 명렁어
        CRUD
        사용 빈도가 가장 높음
        a. SELECT - 조회(읽기) > R
        b. INSERT - 추가(생성) > C
...
create table aaaaaaaaaaaaaaaaaaaaaaaaaaa(
    num number
);
-- 식별자는 30자가 넘어가면 안됨
